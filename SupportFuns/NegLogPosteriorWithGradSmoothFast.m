% function L = NegLogPosteriorWithGradSmoothFast(xAug, R, thetaHatCur, S2HatCur, Q, beta, theta)
...
%           Qsmooth  : Smoothing matrix
%           beta     : weight for smoothing prior
%
...
function [L,g] = NegLogPosteriorWithGradSmoothFast(X1,X0,D, thetaMuPrior, S2InvPrior, Q, beta, theta)
    ...
    L    =  -(sum(L1) + sum(L0)) + (D/2)*log(2*pi) + 0.5*log(1./det(S2InvPrior)) + ...
             ...
             0.5*beta*(theta*Q*theta'); 
   
    D1   = diag(gPrimeMat(X1,theta)./gMat(X1,theta));
    D0   = diag(-gPrimeMat(X0,theta)./(1-gMat(X0,theta)));
    g    = -(sum(D1*X1,1) + sum(D0*X0,1)) + (theta-thetaMuPrior)*S2InvPrior + beta*theta*Q;
